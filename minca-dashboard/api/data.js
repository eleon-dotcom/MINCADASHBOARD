...
const FILE_ID = '1fhGxWE-eYu2PhIMSh3CWLGRUoKph1QzL';
...
function processData(buffer) {
  ...
  const all_ym = Array.from(ymSet).map(k=>{const[y,m]=k.split('-').map(Number);return[y,m];})
  ...
  const filter = (yr,mes,tal) => rows.filter(r=>r.ano===yr&&r.mes===mes&&(!tal||r.taller===tal));

  // vm
  const vm = {};
  all_ym.forEach(([yr,mes])=>{
    const k=`${yr}-${mes}`, all=filter(yr,mes);
    vm[k]={total:Math.round(sumV(all)),ordenes:uniqO(all),talleres:{}};
    TALLERES.forEach(t=>{
      const d=filter(yr,mes,t); const v=Math.round(sumV(d)); const o=uniqO(d); const l=d.length;
  ...
  all_ym.forEach(([yr,mes])=>{
    ...
    tc[k]=Object.entries(byTec).map(([name,t])=>{
      ...
      return{TECNICO:name,TALLER:mainT,v,o,l,tk:o?Math.round(v/o):0,
        lpo:o?+(l/o).toFixed(2):0,
        vs:+((v-(avgT[mainT]||0))/Math.max(avgT[mainT]||1,1)*100).toFixed(1),
        nt:Object.keys(t.talleres).length,tl:sorted.map(([t])=>t),
  ...
  all_ym.forEach(([yr,mes])=>{
    ...
    ['ALL',...TALLERES].forEach(scope=>{
      ...
      const tot=sumV(d);
      tp[k][scope]=Object.values(byP).sort((a,b)=>b.v-a.v).slice(0,15)
        .map(x=>({p:x.p,ti:x.ti,v:Math.round(x.v),c:Math.round(x.c),
  ...
  const dc=filter(VT_LAST_YR,VT_CURR_MES), dp2=filter(VT_PREV_YR,VT_PREV_MES);
  ...
  return {
    meta:{lm:VT_CURR_MES,ly:VT_LAST_YR,pm:VT_PREV_MES,py:VT_PREV_YR,
      yrs:[...new Set(rows.map(r=>r.ano))].sort(),T:TALLERES,M:MES_N,
      all_ym,updated:new Date().toISOString()},
    kpis:{vc,vp:vp2,d:vp2?+((vc-vp2)/vp2*100).toFixed(1):0,
      vyr,oc:uniqO(dc),
